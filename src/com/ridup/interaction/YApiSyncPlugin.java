...
import com.ridup.build.BuildJsonForComponent;
import com.ridup.build.BuildJsonForDubbo;
import com.ridup.build.BuildJsonForIntf;
import com.ridup.build.BuildJsonForYapi;
import com.ridup.constant.ProjectTypeConstant;
import com.ridup.constant.YapiConstant;
import com.ridup.dto.YapiApiDTO;
import com.ridup.dto.YapiDubboDTO;
import com.ridup.dto.YapiResponse;
import com.ridup.dto.YapiSaveParam;
 ...
 * @date: 2019/5/15
 */
public class YApiSyncPlugin extends AnAction {
    ...
    public void actionPerformed(AnActionEvent e) {
        ...
        // 添加开始提示
        Notification notification = notificationGroup.createNotification("Uploading，please wait a moment" , NotificationType.INFORMATION);
        ...
        if (ProjectTypeConstant.dubbo.equals(projectType)) {
            ...
            if (yapiDubboDTOs != null) {
                for (YapiDubboDTO yapiDubboDTO : yapiDubboDTOs) {
                    ...
                    try {
                        // 上传
                        YapiResponse yapiResponse = new YApiSync().uploadSave(yapiSaveParam, null, project.getBasePath());
            ...
            if (yapiApiDTOS != null) {
                for (YapiApiDTO yapiApiDTO : yapiApiDTOS) {
                    ...
                    try {
                        // 上传
                        YapiResponse yapiResponse = new YApiSync().uploadSave(yapiSaveParam, attachUpload, project.getBasePath());
                        if (yapiResponse.getErrcode() != 0) {
                            Notification error = notificationGroup.createNotification("sorry ,upload api error cause:" + yapiResponse.getErrmsg(), NotificationType.ERROR);
                            Notifications.Bus.notify(error, project);
                        } else {
        ...
        // Spring服务、接口等
        else if(ProjectTypeConstant.component.equals(projectType)){
            //获得组件需上传的接口列表 参数对象
            ArrayList<YapiApiDTO> yapiApiDTOS = new BuildJsonForComponent().actionPerformedList(e, attachUpload, returnClass);
            if (yapiApiDTOS != null) {
                for (YapiApiDTO yapiApiDTO : yapiApiDTOS) {
                    YapiSaveParam yapiSaveParam = new YapiSaveParam(projectToken, yapiApiDTO.getTitle(), yapiApiDTO.getPath(), yapiApiDTO.getParams(), yapiApiDTO.getRequestBody(),
                        yapiApiDTO.getResponse(), Integer.valueOf(projectId), yapiUrl, true, yapiApiDTO.getMethod(),
                        yapiApiDTO.getDesc(), yapiApiDTO.getHeader());
                    yapiSaveParam.setReq_body_form(yapiApiDTO.getReq_body_form());
                    yapiSaveParam.setReq_body_type(yapiApiDTO.getReq_body_type());
                    yapiSaveParam.setReq_params(yapiApiDTO.getReq_params());
                    yapiSaveParam.setStatus(yapiApiDTO.getStatus());
                    ...
                    if(StringUtils.isBlank(yapiApiDTO.getPath())){
                        yapiSaveParam.setPath("/pending/"+ new Date().getTime());
                    }else{
                        yapiSaveParam.setPath(yapiApiDTO.getBasePath()+yapiApiDTO.getPath());
                    }
                    ...
                    try {
                        ...
                        if (yapiResponse.getErrcode() != 0) {
                            ...
                            String url = yapiUrl + "/project/" + projectId + "/interface/api/cat_" + yapiResponse.getCatId();
                            this.setClipboard(url);
                            Notification error = notificationGroup.createNotification("success ,url:  " + url,
                                NotificationType.INFORMATION);
                            Notifications.Bus.notify(error, project);
                        }
                    } catch (Exception e1) {
                        Notification error = notificationGroup.createNotification("sorry ,upload api error cause:" + e1,
        ...
        // Spring服务、接口等
        else if(ProjectTypeConstant.intf.equals(projectType)){
            //获得组件需上传的接口列表 参数对象
            ArrayList<YapiApiDTO> yapiApiDTOS = new BuildJsonForIntf().actionPerformedList(e, attachUpload, returnClass);
            if (yapiApiDTOS != null) {
                for (YapiApiDTO yapiApiDTO : yapiApiDTOS) {
                    YapiSaveParam yapiSaveParam = new YapiSaveParam(projectToken, yapiApiDTO.getTitle(), yapiApiDTO.getPath(), yapiApiDTO.getParams(), yapiApiDTO.getRequestBody(),
                        yapiApiDTO.getResponse(), Integer.valueOf(projectId), yapiUrl, true, yapiApiDTO.getMethod(),
